...
## in FASTQ format, by performing alignment to a chimeric mouse-human (mm10-hg38) reference
## genome and then removing all reads that more closely align to mouse reads.
##
## Requirements/expectations :
## - PDX sample reads in FASTQ format
...
workflow FastqCleanPDX {
	...
	# Align the input FASTQ file to the chimeric mouse-human genome
	call BwaMem {
    input:
...
task BwaMem {
  ...
  Float fastq_size = size(input_fastq, "GB")
  ...
  Int disk_size = ceil(fastq_size + 16 + (3 * fastq_size) + 100)
    ...
    /usr/gitc/~{bwa_commandline} ~{input_fastq} - 2> >(tee ~{output_bam_basename}.bwa.stderr.log >&2) | \
