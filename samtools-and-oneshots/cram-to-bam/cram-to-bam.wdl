...
##
## Update Notes:
## Updated by Aparicio Lab (BC Cancer Research Centre) June 2022 to optimize runtime parameters for Cromwell on Azure implementation (instead of Google Cloud). 
## Also changed all references from "sample_name" to "sample_ID" for consistency with other Aparicio Lab pipelines.
...
task CramToBamTask {
  ...
  output {
    File outputBam = "~{sample_ID}.bam"
    File outputBai = "~{sample_ID}.bai"
  }
}
